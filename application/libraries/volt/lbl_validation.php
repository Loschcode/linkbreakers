...
class LBL_Validation extends CI_Context {
	...
	/**
	 *
	 * Static variable, don't forget to add one if we check another field than string or url
	 *
	 */
	static $lbl_validation_return_string = TRUE;
	static $lbl_validation_return_url = TRUE;

	static $lbl_validation_return_space_description = TRUE;
	static $lbl_validation_return_space_redirection =  TRUE;

	static $error_message = '';

	/**
	 * Check the LBL rules within this file and return a static variable with the (bool) result
	 *
	 * @access	public
	 * @param string the $str we are looking through
	 * @return bool
	 */
	public function check_rules($field, $rules) {

		$validation_name = 'lbl_validation_return_'.$field;

		self::$$validation_name = TRUE;

		$rules_array = explode('|', $rules);

		$argument = $this->input->post($field);

		// We trim everything to check if it's empty
		$empty_check = clean(trim($this->panda->trim_lines($argument)));

		if (empty($empty_check)) return FALSE;

		foreach ($rules_array as $rule_function) {

			$return = $this->$rule_function($argument);

			// There's an invalid rule, we need to break and set the global variable
			if ($return === FALSE) {

				self::$$validation_name = FALSE;
				break;
	...
	/**
	 * Set a (static) error message we could transfert to CI form_validation after
	 *
	 * @access	public
	 * @param string $fun the function that generated the error
	 * @param string $msg the error message we want to show (including possibly %s for the field through CI)
	 * @return void
	 */
	public function set_error_message($fun, $msg) {
	...
	public function secure_dyn_url($str) {
		...
		if (!$this->panda->secure_dyn_url($str)) {
			...
			$this->set_error_message('secure_dyn_url', 'This %s entry contains forbidden characters.');
			return FALSE;
	...
	public function secure_string($str) {
		...
		if (strpos($str, '"')) {
			...
			$this->set_error_message('secure_string', 'This %s entry contains forbidden characters (").');
			return FALSE;
	...
	public function double_spaces($str) {
		...
		if (strpos($str, "  ")) {
			...
			$this->set_error_message('double_spaces', 'The %s field cannot contain double spaces between words');
			return FALSE;
	...
	public function var_restriction($str) {
		...
		if ($num_dyn_words >= $num_words) {
			...
			$this->set_error_message('var_restriction', 'The %s field cannot contain only dynamic datas');
			return FALSE;
	...
	public function tools_check($str) {
		...
		if ($str[0] === '#') {
			...
			elseif ($this->panda->existing_tool($first_word_with_return)) return TRUE;
				...
				$this->set_error_message('tools_check', 'The %s field contains unknown tool (#'.$first_word.')');
				return FALSE;
	...
	public function var_string_format($str) {
		...
		$this->set_error_message('var_string_format', 'The %s field contains wrong variable format - note: $variable[type]');
	...
	public function var_url_format($str) {
		...
		$this->set_error_message('var_url_format', 'The %s field contains wrong variable format - note: $variable(format)');
	...
	public function comments_format($str) {
		...
		if ($start_comment === $stop_comment) {
			...
			$this->set_error_message('comments_format', 'The %s field contains invalid comments structure ('.LBL_LEFT_COMMENT.' example '.LBL_RIGHT_COMMENT.')');
			return FALSE;
	...
	public function brackets_format($str) {
		...
		if ($start_bracket === $stop_bracket) {
			...
			$this->set_error_message('brackets_format', 'The %s field contains invalid {FUNCTION} format');
			return FALSE;
	...
	public function lbl_function_existence($str) {
		...
		foreach ($functions_list as $function) {
			...
			if (isset($result['error'])) {
				...
				$this->set_error_message('lbl_function_existence', 'The %s field contains an unknown function ('.$function.')');
				return FALSE;
	...
	public function check_conditions_structure($str) {
		...
		*/
		...
		if ($error) {
			...
			$this->set_error_message('check_conditions_structure', $validation_message.' ('.$validation_message_detail.')');
			return FALSE;
	...
	public function var_alone($str2) {
		...
		$this->set_error_message('var_alone', 'The %s field contains single variables or miss some of them, check your variables names ('.$check.' are missing)');
	...
	public function check_linkedurl($str) {
		...
		if ($this->general_model->find_id_by_autocomplete($autocomplete, $iduser) === 0) {
			...
			$this->set_error_message('check_linkedurl', 'This linked-%s entry didn\'t find its link, impossible to validate this entry.');
			return FALSE;
